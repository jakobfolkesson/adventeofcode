...
posible_obsticles = set(positions)
posible_obsticles.remove(start_pos)

create_loops = []

for index,pos in enumerate(posible_obsticles):
    l = Level(input)
    l.map[pos.y][pos.x] = '#'
    ...
    while not oob:
        ...
        past_pos = pos_dirs[dir]

        # check if any steps are in past_pos
        if any([step in past_pos for step in steps]):
            create_loops.append(pos)
            break
        else:
            pos_dirs[dir].extend(steps)

print(len(create_loops))